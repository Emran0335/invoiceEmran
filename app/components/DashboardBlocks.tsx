...
async function getData(userId: string) {
  const [data, openInvoices, padiInvoices] = await Promise.all([
...
export default async function DashboardBlock() {
  ...
  const { data, openInvoices, padiInvoices } = await getData(
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 md:gap-8">
      ...
      <Card>
        ...
        <CardContent>
          <h2 className="text-2xl font-bold">+{padiInvoices.length}</h2>
          <p className="text-xs text-muted-foreground">
